...
export function FormEditorPage({ 
  ...
  const [stepperType, setStepperType] = useState<'dots' | 'numbers' | 'progress' | 'breadcrumb'>('progress');
  ...
  React.useEffect(() => {
    if (schema && schema.layout !== selectedTemplate) {
      ...
      onSchemaUpdate({
        ...schema,
        layout: selectedTemplate as 'simple' | 'two-column' | 'wizard' | 'carded'
      });
  ...
  const templates = [
    ...
    { id: 'wizard', name: 'Wizard', preview: '①②③', description: 'Multi-step form' },
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex relative">
      ...
      <div className={`border-r border-border bg-card flex flex-col transition-all duration-300 ${leftPanelCollapsed ? 'w-0 border-r-0' : 'w-80'} overflow-hidden`}>
        {!leftPanelCollapsed && (
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      className={`border-2 rounded-[var(--radius-button)] ${isListening ? 'border-primary bg-primary/10' : 'border-border'}`}
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['sharp', 'rounded', 'pill'] as const).map((type) => (
                    ...
                    >
                      <div className={`h-8 mx-auto mb-1 bg-primary/20 border-2 border-primary/40 ${
                        type === 'sharp' ? 'rounded-none' : type === 'rounded' ? 'rounded-md' : 'rounded-full'
                      }`} />
                      <p className="text-foreground text-xs capitalize">{type}</p>
                    </button>
                  ))}
                </div>
              </div>
                  ...
                  {(['compact', 'comfortable', 'spacious'] as const).map((type) => (
                    ...
                    >
                      <div className="space-y-1 mb-1">
                        <div className={`h-1.5 bg-primary/40 rounded-sm ${type === 'compact' ? 'mx-4' : type === 'comfortable' ? 'mx-2' : 'mx-0'}`} />
                        <div className={`h-1.5 bg-primary/40 rounded-sm ${type === 'compact' ? 'mx-4' : type === 'comfortable' ? 'mx-2' : 'mx-0'}`} />
                      </div>
                      <p className="text-foreground text-xs capitalize">{type}</p>
                    </button>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['top', 'left', 'inline'] as const).map((type) => (
                    ...
                    >
                      <div className={`mb-1 ${type === 'left' ? 'flex items-center gap-1' : 'space-y-1'}`}>
                        <div className="h-1 bg-primary/40 rounded-sm" style={{ width: type === 'inline' ? '60%' : type === 'left' ? '40%' : '100%' }} />
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['sm', 'md', 'lg'] as const).map((size) => (
                    ...
                    >
                      <div className={`bg-primary/20 border border-primary/40 rounded-sm mx-auto mb-1 ${
                        size === 'sm' ? 'h-4 w-[70%]' : size === 'md' ? 'h-6 w-[80%]' : 'h-8 w-[90%]'
                      }`} />
                      <p className="text-foreground text-xs uppercase">{size}</p>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex items-center gap-1 p-1 bg-secondary rounded-[var(--radius)]">
              <Button
                ...
                className={`rounded-[var(--radius-sm)] ${
                  viewportMode === 'desktop' ? 'bg-primary text-primary-foreground' : ''
                }`}
              ...
              <Button
                ...
                className={`rounded-[var(--radius-sm)] ${
                  viewportMode === 'tablet' ? 'bg-primary text-primary-foreground' : ''
                }`}
              ...
              <Button
                ...
                className={`rounded-[var(--radius-sm)] ${
                  viewportMode === 'mobile' ? 'bg-primary text-primary-foreground' : ''
                }`}
            ...
            <div className="p-8">
              ...
              >
                <FormRenderer 
                  schema={schema} 
                  onFormDataChange={() => {}} 
                  template={selectedTemplate as 'simple' | 'two-column' | 'wizard' | 'carded'}
                  themeColors={themes.find(t => t.id === selectedTheme)?.colors}
      ...
      <div className={`border-l border-border bg-card transition-all duration-300 ${rightPanelCollapsed ? 'w-0 border-l-0' : 'w-96'} overflow-hidden`}>
        {!rightPanelCollapsed && (
      ...
      <Dialog open={showFieldsList} onOpenChange={setShowFieldsList}>
        <DialogContent className="bg-card border-border rounded-[var(--radius-card)] max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-foreground">All Fields ({schema.fields.length})</DialogTitle>
      ...
      <Dialog open={showStepsDialog} onOpenChange={setShowStepsDialog}>
        <DialogContent className="bg-card border-border rounded-[var(--radius-card)]">
          <DialogHeader>
            <DialogTitle className="text-foreground">Wizard Steps Navigation</DialogTitle>
            <DialogDescription className="text-muted-foreground">
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 rounded-[var(--radius)] border-2 transition-all text-left hover:border-primary hover:bg-primary/5 ${
                    wizardStep === step ? 'border-primary bg-primary/5' : 'border-border bg-background'
                  }`}
                >
