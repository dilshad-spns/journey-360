...
import { FileText, Mic, Upload, Sparkles, Clock } from 'lucide-react';

interface LandingPageProps {
  onSelectMode: (mode: 'text' | 'speech' | 'upload', prefilledRequirements?: string) => void;
}
  ...
  const inputModes = [
    {
      id: 'text' as const,
      title: 'Paste as Text',
      description: 'Copy and paste your requirements directly',
      icon: FileText,
    ...
    },
    {
      ...
      description: 'Speak your requirements using voice input',
      icon: Mic,
    ...
    },
    {
      ...
      description: 'Upload a file with your requirements',
      icon: Upload,
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-background">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center h-20 w-20 rounded-[var(--radius-card)] bg-[rgba(251,146,60,0)] mb-4">
          ...
          <h1 className="text-foreground mb-2">Journey 360</h1>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Choose how you'd like to share your requirements and let AI build your deployable journey
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-6xl mx-auto mb-8">
          {inputModes.map((mode) => {
            ...
            return (
              <Card
                key={mode.id}
                className="p-5 bg-card border border-border rounded-[var(--radius-card)] hover:border-primary transition-all cursor-pointer group flex flex-col"
              ...
              >
                <div className={`h-12 w-12 rounded-[var(--radius-card)] ${mode.iconBg} flex items-center justify-center mb-4 group-hover:scale-105 transition-transform`} aria-hidden="true">
                  <Icon className={`h-6 w-6 ${mode.iconColor}`} />
                </div>
                <h3 className="text-foreground mb-2 break-words">{mode.title}</h3>
                <p className="text-muted-foreground mb-4 break-words">{mode.description}</p>
                <Button
                  className="w-full bg-primary text-primary-foreground rounded-[var(--radius-button)] hover:bg-primary/90 transition-all mt-auto"
                  onClick={(e) => {
                    e.stopPropagation();
                    onSelectMode(mode.id);
                  }}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-foreground">Recent Prompts</h3>
          </div>
          <div className="grid gap-2">
            {recentPrompts.map((prompt, index) => (
              <button
                key={index}
                onClick={() => onSelectMode('text', prompt)}
                className="text-left p-3 bg-card border border-border rounded-[var(--radius-card)] hover:border-primary hover:bg-primary/5 transition-all group w-full"
                aria-label={`Use recent prompt: ${prompt}`}
              >
                <div className="flex items-center justify-between gap-3">
                  <p className="text-foreground group-hover:text-primary transition-colors break-words flex-1">
                    {prompt}
                  </p>
                  <Badge className="bg-primary text-primary-foreground rounded-[var(--radius-pill)] opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" aria-hidden="true">
                    Use
                  </Badge>
                </div>
              </button>
