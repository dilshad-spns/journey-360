...
export function FormRenderer({
  ...
  // Fetch dynamic options for fields with apiConfig
  useEffect(() => {
    if (!schema) return;

    const fetchOptionsForFields = async () => {
      const fieldsWithApi = schema.fields.filter(
        (field) =>
          field.apiConfig && (field.type === "select" || field.type === "radio")
      );

      for (const field of fieldsWithApi) {
        if (!field.apiConfig) continue;
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      // Check if schema has a submitUrl
      if (schema.submitUrl) {
        // Make actual API call to external server
        const response = await ApiService.submitFormData(
          ...
          data,
          {
            method: "POST",
            headers: {
              Authorization: "Bearer YOUR_TOKEN_HERE", // Add auth if needed
            },
          }
        );

        console.log("Form submitted to server:", response);
        ...
        // Fallback to simulation if no submitUrl
  ...
  const renderField = (field: FieldSchema) => {
    ...
    switch (field.type) {
      ...
      case "select":
        ...
        const isLoadingSelect = loadingFields.has(field.name);
        ...
        return renderFieldWrapper(
          ...
          >
            ...
            <SelectContent className='bg-card border-border rounded-[var(--radius)]'>
              {selectOptions.length === 0 && !isLoadingSelect ? (
      ...
      case "radio":
        const radioOptions = dynamicOptions[field.name] || field.options || [];
        const isLoadingRadio = loadingFields.has(field.name);
