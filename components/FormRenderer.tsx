  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (Math.random() > 0.1) {
        ...
        // If wizard is enabled and there's a next step (like confirmation), move to it
        if (showStepper && wizardStep < wizardSteps.length - 1) {
          setTimeout(() => {
          ...
          // Otherwise reset form after delay
          setTimeout(() => {
            reset();
            setSubmitSuccess(false);
            // Reset to first step if wizard is enabled
            if (showStepper) {
              setWizardStep(0);
            }
          }, 3000);
          ...
          <div className='flex gap-3 pt-6 border-t border-border'>
            ...
            {showStepper && wizardStep < wizardSteps.length - 1 ? (
              ...
              currentStepFields.every(
                (f) =>
                  f.type === "heading" ||
                  (f.type === "text" && !f.validations?.length)
              ) ? (
              <Button
                type='button'
                onClick={() => {
