...
export function InputRequirementScreen({ mode, onBack, onContinue, initialRequirements = '' }: InputRequirementScreenProps) {
  ...
  const [selectedImportType, setSelectedImportType] = useState<string>('Upload');
  ...
  useEffect(() => {
    ...
    console.log('🔍 InputRequirementScreen: initialRequirements =', initialRequirements);
    console.log('🔍 InputRequirementScreen: current input state =', input);
    
    if (initialRequirements) {
      console.log('✅ InputRequirementScreen: Setting input to initialRequirements:', initialRequirements);
      setInput(initialRequirements);
    } else {
      console.log('ℹ️ InputRequirementScreen: No initial requirements provided');
  ...
  const toggleRecording = async () => {
    ...
    if (isRecording) {
      ...
      if (!isSecureContext) {
        setSpeechError('Microphone access requires HTTPS. Please access this page via HTTPS or use localhost for development.');
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        setSpeechError('Your browser doesn\'t support microphone access. Please use a modern browser like Chrome, Edge, or Safari.');
        setMicPermission('denied');
        return;
      }
      ...
      try {
        ...
        if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
          ...
          if (userAgent.includes('chrome') && !userAgent.includes('edge')) {
            helpText = 'Chrome: Click the 🔒 or 🎤 icon in the address bar, find "Microphone", select "Allow", then refresh the page.';
          } else if (userAgent.includes('edge')) {
            helpText = 'Edge: Click the 🔒 icon in the address bar, find "Microphone", select "Allow", then refresh the page.';
          } else if (userAgent.includes('safari')) {
            helpText = 'Safari: Go to Safari > Settings for This Website > Microphone > Allow, then refresh the page.';
          } else if (userAgent.includes('firefox')) {
            helpText = 'Firefox: Click the 🔒 icon > Connection Secure > More Information > Permissions > Use the Microphone > Allow.';
          } else {
            helpText = 'Please check your browser settings to allow microphone access for this site.';
          }
          
          setSpeechError(`${errorMessage} ${helpText}`);
        } else if (error.name === 'NotFoundError' || error.name === 'DevicesNotFoundError') {
          setSpeechError('No microphone found. Please connect a microphone to your device and try again.');
        } else if (error.name === 'NotSupportedError') {
          setSpeechError('Your browser doesn\'t support microphone access. Please use Chrome, Edge, or Safari.');
        } else if (error.name === 'NotReadableError' || error.name === 'TrackStartError') {
          setSpeechError('Your microphone is already in use by another application. Please close other apps using the microphone and try again.');
        } else if (error.name === 'OverconstrainedError') {
          setSpeechError('Unable to access microphone with the requested settings. Please try again.');
        } else if (error.name === 'SecurityError') {
          setSpeechError('Microphone access blocked due to security settings. Please ensure you\'re accessing the site via HTTPS.');
        } else {
          setSpeechError(`Unable to access microphone: ${error.message || error.name}. Please check your browser and system settings.`);
        }
        return;
      }
      ...
      if (typeof window !== 'undefined') {
        ...
        if (SpeechRecognition) {
          ...
          setSpeechError('Speech recognition is not supported in your browser. Please use Chrome, Edge, or Safari, or type your requirements instead.');
          setIsRecording(false);
          if (timerRef.current) {
            clearInterval(timerRef.current);
            timerRef.current = null;
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      setInput(`Error processing ${file.name}. Please try a different file or paste your requirements manually.`);
      setIsProcessingFile(false);
    }
  };
  ...
  const correctGrammar = () => {
    ...
    correctedText = correctedText.replace(/(^\w|[.!?]\s+\w)/g, (match) => match.toUpperCase());
    ...
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="relative container mx-auto px-6 py-6">
          ...
          <div className="mb-6 p-4 bg-destructive/10 border border-destructive/20 rounded-[var(--radius-card)] max-w-4xl mx-auto">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-destructive mb-1">Microphone Access Required</h4>
                <p className="text-muted-foreground text-sm mb-2">
                  Speech recognition requires microphone access. Please enable it in your browser settings.
                </p>
                <details className="text-xs text-muted-foreground">
                  ...
                  <div className="mt-2 space-y-2 pl-4 border-l-2 border-border">
                    <p><strong>Chrome/Edge:</strong> Click the 🔒 icon in address bar → Microphone → Allow → Refresh</p>
                    <p><strong>Safari:</strong> Safari menu → Settings for This Website → Microphone → Allow → Refresh</p>
                    <p><strong>Firefox:</strong> Click 🔒 icon → More Information → Permissions → Microphone → Allow</p>
                  </div>
                </details>
              </div>
            </div>
        ...
        {typeof window !== 'undefined' && !window.isSecureContext && window.location.hostname !== 'localhost' && (
          <div className="mb-6 p-4 bg-destructive/10 border border-destructive/20 rounded-[var(--radius-card)] max-w-4xl mx-auto">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-destructive mb-1">Secure Connection Required</h4>
                <p className="text-muted-foreground text-sm">
                  Microphone access requires HTTPS. Please access this page via <code className="px-1 py-0.5 bg-muted rounded text-xs">https://</code> instead of <code className="px-1 py-0.5 bg-muted rounded text-xs">http://</code>
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-foreground mb-3 text-[20px] font-bold">What would you like to create?</h1>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Describe your journey in natural language, and we'll generate a complete deployable form with UI, validations, and tests.
          </p>
        </div>
                    ...
                    <div className="flex-1 relative">
                      {mode === 'upload' && !input ? (
                        ...
                        <div
                          ...
                          `}
                        >
                          <div className={`h-16 w-16 rounded-full bg-primary/10 flex items-center justify-center transition-transform ${isDragging ? 'scale-110' : ''}`}>
                            <Upload className="h-8 w-8 text-primary" />
                          </div>
                          <div className="text-center">
                            <p className="text-foreground mb-1">
                              {isProcessingFile ? 'Processing file...' : isDragging ? 'Drop your file here' : 'Drag and drop your file here'}
                            </p>
                            <p className="text-muted-foreground text-sm">
                              or click to browse
                            </p>
                        ...
                        <div className="w-full h-[250px] flex flex-col items-center justify-center">
                          ...
                          <button
                            ...
                            className={`
                              ...
                              ${!isRecording && micPermission !== 'denied' ? 'animate-pulse' : ''}
                          ...
                          >
                            ...
                            {isRecording ? (
                              ...
                              <Mic className={`h-16 w-16 relative z-10 ${
                                micPermission === 'denied' ? 'text-destructive' : 'text-primary'
                              }`} />
                            )}
                            ...
                            {isRecording ? (
                              <div className="flex flex-col items-center gap-2">
                                <div className="flex items-center gap-2 px-4 py-2 bg-destructive/10 border border-destructive/20 rounded-[var(--radius-pill)]">
                                  ...
                                  <span className="text-destructive">Recording {formatTime(recordingTime)}</span>
                                </div>
                                <p className="text-muted-foreground text-sm">Click microphone to stop</p>
                              </div>
                            ) : micPermission === 'denied' ? (
                              <div className="space-y-1">
                                <p className="text-destructive">Microphone access denied</p>
                                <p className="text-muted-foreground text-sm">Please enable in browser settings</p>
                              </div>
                            ) : (
                              <div className="space-y-1">
                                <p className="text-foreground">Click to start narrating</p>
                                <p className="text-muted-foreground text-sm">Your voice will be transcribed in real-time</p>
                              </div>
                            )}
                          </div>
                          ...
                          {transcriptText && (
                            <div className="mt-4 px-4 py-2 bg-primary/5 border border-primary/20 rounded-[var(--radius-card)] max-w-md">
                              <p className="text-primary text-sm italic text-center">"{transcriptText}"</p>
                            </div>
                          )}
                        </div>
                      ) : (
                      ...
                      {mode === 'speech' && input && (
                        <div className="mt-4 space-y-2">
                          <div className="flex items-center justify-between">
                            <label className="text-muted-foreground text-sm">Transcribed Text:</label>
                            <button
                              type="button"
                              onClick={() => setInput('')}
                      ...
                      {(isRecording || uploadedImage || isProcessingFile || speechError) && (
                          ...
                          {speechError && (
                            <div className="p-3 bg-destructive/10 border border-destructive/20 rounded-[var(--radius-card)]">
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                  <p className="text-destructive text-sm mb-2">{speechError}</p>
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <Button
                                      size="sm"
                                      variant="outline"
                                      onClick={() => window.open('https://www.onlinemictest.com/', '_blank')}
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        >
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className="text-muted-foreground text-xs">Import from web URL</div>
                            </div>
                          </DropdownMenuItem>
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className="text-muted-foreground text-xs">Import Jira ticket</div>
                            </div>
                          </DropdownMenuItem>
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className="text-muted-foreground text-xs">Import GitHub repo</div>
                            </div>
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                      ...
                      {mode === 'speech' && (
                        <div className="flex-1 flex items-center justify-center">
                          <p className="text-muted-foreground text-sm">Click the microphone above to start narrating</p>
                        </div>
                      )}
              ...
              <div className="relative p-3.5 bg-muted border border-border rounded-[var(--radius-card)] hover:border-border-hover transition-all duration-300 overflow-hidden">
                <div className="relative">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-foreground" style={{ fontSize: '0.813rem' }}>
                      {mode === 'speech' ? 'Example Narration' : mode === 'upload' ? 'Supported Files' : 'Example Input'}
                    </h4>
                  </div>
                  <p className="text-foreground italic" style={{ fontSize: '0.813rem' }}>
                    {mode === 'speech' 
        ...
        <DialogContent className="bg-card border-border rounded-[var(--radius-card)] max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              Paste code snippets that describe your form requirements or structure. The code will be added to your input.
            </DialogDescription>
          </DialogHeader>
